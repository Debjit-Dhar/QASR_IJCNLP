...
from collections import Counter  # <-- added for majority voting
...
def evaluate_pretrained_model(model, test_loader, device='cpu', dataset_name='google', n_repeats=10):
    """Evaluate the pretrained model.
       If dataset_name == 'google', run each sample through the model n_repeats times
       and select the most frequent predicted class (majority vote).
    """
    ...
    with torch.no_grad():
        for audio, labels in tqdm(test_loader, desc="Evaluating Pretrained Model"):
            ...
            if dataset_name.lower() == 'google':
                # Repeat inference n_repeats times and majority-vote per-sample
                batch_size = labels.size(0)
                preds_per_rep = np.zeros((n_repeats, batch_size), dtype=np.int64)
                
                for r in range(n_repeats):
                    outputs = model(audio)  # (batch, num_classes)
                    _, predicted = torch.max(outputs.data, 1)
                    preds_per_rep[r, :] = predicted.cpu().numpy()
                
                # Majority vote per sample
                final_preds = []
                for i in range(batch_size):
                    cnt = Counter(preds_per_rep[:, i])
                    most_common_label, _ = cnt.most_common(1)[0]
                    final_preds.append(int(most_common_label))
                final_preds = np.array(final_preds, dtype=np.int64)
                
                total += batch_size
                correct += int((final_preds == labels.cpu().numpy()).sum())
                
                predictions.extend(final_preds.tolist())
                targets.extend(labels.cpu().numpy().tolist())
            
            else:
                # Original single-pass evaluation (for LibriSpeech)
                outputs = model(audio)
                _, predicted = torch.max(outputs.data, 1)
...
def main():
    ...
    parser.add_argument('--n_repeats', type=int, default=10, help='Number of repetitions for majority voting (google only, default: 10)')
    ...
    print(f"Repeats for majority voting (google only): {args.n_repeats}")
    ...
    wer, accuracy = evaluate_pretrained_model(model, test_loader, device, dataset_name=args.dataset.lower(), n_repeats=args.n_repeats)
